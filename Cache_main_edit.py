...
def main():
    ...
    with tf.Session() as sess:
        ...
        for episode in range(max_episodes):
            ...
            for i in range(request * env.Num_packet):
                ...
                # print(np.argmax(mainDQN.predict(state)[0]))
                ...
                # print("action(SBS):", action // (env.Memory))
                # print("action(packet):", env.BS[action // (env.Memory)][action % (env.Memory)])
            ...
            if episode > 30000 and env.cost > 12000:
                for i in range(49999200, 50000000, 1):
            ...
            if episode % 100 == 99:
                ...
                # experience_batch = replay_buffer.copy()
                # np.random.shuffle(experience_batch)
                for _ in range(20):
                    minibatch = rd.sample(replay_buffer, 2500)
    ...
    np.save("Y_(sample)", y_layer) #"Y_(act %)"
